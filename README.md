...
│       │   ├─ ages.js             年齢（NORMAL）
│       │   ├─ integers.js         整数（HARD）
│       │   ├─ speed-distance.js   速さ・時間（HARD）
│       │   ├─ catch-up.js         追いつき・出会い（NORMAL）
│       │   ├─ percentage.js       割合・増減（HARD）
│       │   ├─ two-products.js     2種類の品物（NORMAL）
...
│           ├─ open-box.js              箱の容積（NORMAL、図あり）
│           ├─ moving-points.js         動点（HARD、図あり）
│           ├─ price-sales.js           価格と売上（HARD）
│           └─ profit-discount.js       利益と割引（HARD、分数UIを使用）
│
└─ assets/
...
- 中1「1次方程式」全11カテゴリ（個数・代金／所持金・過不足／分配・過不足／
  長いす・過不足／年齢／整数／速さ・時間／追いつき・出会い／
  割合・増減／2種類の品物／人数と料金）、合計33種類以上の問題テンプレート
- 中2「連立方程式」全12カテゴリ（2種類の品物／人数と料金／硬貨・紙幣／
  2けたの自然数／年齢／速さ・道のり／電車の通過／池の周回／食塩水の混合／
  割合の増減・人数／割合の増減・代金／平均）、合計36種類以上の問題テンプレート
- 中3「2次方程式」全9カテゴリ（連続する整数の積／数とその平方／長方形の面積／
  面積の増減／面積・十字路／箱の容積／動点／価格と売上／利益と割引）、
...
- 正誤判定は標準形どうしの比例判定でしかないため、`c(x−d)²=V`と`c(x−d)(x−d)=V`は
  ...
  （`箱の容積`カテゴリの模範式・模範式表示は、第4段階の途中でこの新しい
  `c(x−d)²=V`表記に統一した）。
...
`cross-road`（面積・十字路）・`open-box-net`（箱の容積）・`moving-points-rectangle`（動点）
...
- `diagram`が`null`の問題（十字路・ふたのない箱・動点以外の6カテゴリ、および「箱の容積」の
...
（動点カテゴリの`1/2`、利益と割引カテゴリの`x/10`で使用）。
...
- `type: "fraction"`を指定すると、上下型分数の見た目のボタンになり、押したときに
  ...
  詳細は「分数入力」を参照。
- `hint`の文章中に分数（例：`x/10`）を含める場合は、生の文字列のまま埋め込まず、
  `{{fraction:分子:分母}}`（例：`{{fraction:x:10}}`）と書く。`ui.js`の
  `appendTextWithInlineFractions()`が、ヒントパネル（ゲーム画面）・例題確認画面の両方で、
  これを上下型分数として描画する（「このゲームの遊び方」の本文で使っている記法と同じ）。
  生の`x/10`のまま書くと、画面には文字どおり「x/10」と表示されてしまうので注意する
  （2次方程式「利益と割引」のヒントで使用）。
...
共有の画面（`quest-view-summary`）で表示するが、見出しは「リタイアしました」のみとし、
上記の補足メッセージは表示しない（`quest-summary-message`はメッセージが空文字の場合、
要素ごと非表示になる）。たどったルートは画面上部の簡易マップに表示済みのため、
結果画面では重複して表示しない。
...
- **単元・カテゴリ選択**：`js/help/example-ui.js`が持つローカル状態
  ...
  `@media (min-width: 768px)`で出し分け）。
- **2つの入口と「もどる」の戻り先**：ヘルプメニューの「例題確認」から開いた場合
  （`openExampleCatalogFromHelpMenu()`）は「1次方程式・先頭カテゴリ」から始まり、
  「もどる」を押すとヘルプメニューへ戻る。ゲーム画面のヒントウィンドウの「例題確認」から
  開いた場合（`openExampleCatalogForQuestion(unit, categoryId)`、下記「ヒントウィンドウの
  『例題確認』」を参照）は、現在出題されているカテゴリから始まり、「もどる」を押すと
  問題画面へ戻る。「← ○○にもどる」ボタンの文言・右上の小さな「もどる」ボタンの
  `aria-label`は、どちらの入口から開いたかに応じて動的に切り替わる
  （`applyReturnLabels()`）。
...
- **データの解決・検証（`js/help/example-catalog.js`）**：
  ...
  `validateQuestion()`をまとめて検証する（問題があってもコンソール警告のみ）。収録数は
  `getTotalCategoryCount()`が`getCategoriesForUnit(unit).length`の合計から動的に計算できる
  （画面表示はしていないが、コード内に`32`という数値をハードコードしていない）。
...
問題画面のヒントウィンドウ（「閉じる」ボタンの隣）にも「例題確認」ボタンを配置しており、
押すと現在出題されているカテゴリの代表例題を、上記と同じ例題確認画面で確認できる。

- **時間制限があるモードでは出現しない**：段位認定・クエストのように制限時間があるモードでは、
  ヒントを開いても「例題確認」ボタン自体が表示されない
  （`ui.setHintExampleButtonVisible(gameState.mode === "training")`をヒント表示時に呼ぶ）。
  トレーニングは制限時間がないため、常に表示される。
- **現在のカテゴリで開く**：`openExampleCatalogForQuestion(gameState.unit,
  gameState.currentQuestion.categoryId)`を呼び、単元・カテゴリをその場で設定して描画する
  （ヘルプメニューからの「1次方程式・先頭カテゴリ」への初期化は行わない）。対応する
  カテゴリが見つからない場合は、その単元の先頭カテゴリにフォールバックする。
- **問題画面に戻る**：「もどる」を押すと、`ui.showScreen("game")`で問題画面（同じ問題・
  同じ入力内容）へ戻る。ヒントウィンドウを開いたまま画面遷移すると閉じた状態のまま残って
  しまうため、例題確認へ移動する直前に`ui.hideHintPanel()`を呼んでいる。
- **gameStateへの影響なし**：例題確認画面の閲覧・カテゴリ切り替えは、通常のヘルプメニュー
  経由と同様に`gameState`・タイマー・スコア・問題履歴・localStorageに一切影響しない。
...
| 難易度 | 出題されるカテゴリ |
| --- | --- |
| NORMAL | 個数・代金／分配・過不足／年齢／追いつき・出会い／2種類の品物／人数と料金 |
| HARD | 全11カテゴリ（NORMALの6カテゴリ＋所持金・過不足／長いす・過不足／整数／速さ・時間／割合・増減） |

### 中2「連立方程式」

| 難易度 | 出題されるカテゴリ |
| --- | --- |
| NORMAL | 2種類の品物／人数と料金／硬貨・紙幣／年齢／速さ・道のり／食塩水の混合 |
| HARD | 全12カテゴリ（NORMALの6カテゴリ＋2けたの自然数／電車の通過／池の周回／割合の増減・人数／割合の増減・代金／平均） |

### 中3「2次方程式」

| 難易度 | 出題されるカテゴリ |
| --- | --- |
| NORMAL | 連続する整数の積／数とその平方／長方形の面積／面積の増減／面積・十字路／箱の容積 |
| HARD | 全9カテゴリ（NORMALの6カテゴリ＋動点／価格と売上／利益と割引） |
...
- 数式キーボードに「□²」ボタンが、9カテゴリすべてで表示されること。「(」「x」「−」「4」「)」
  ...
  1回で削除され「(x−4)」が残ること
- 箱の容積（L3-06）で、模範式が`c(x−d)²＝V`という上付き表記で表示されること
  （`c(x−d)(x−d)＝V`という展開形の表記には戻らないこと）
- 箱の容積で、`c(x-d)^2=V`・`c(x-d)²=V`・`c(x-d)(x-d)=V`のいずれの入力も正解になること
...
- 次の入力が入力エラーになり、不正解回数へ加算されないこと：
  ...
  `x²/0=1` `x^^2=4` `x^3=8`
- 面積・十字路／箱の容積／動点の3カテゴリで、問題文カードの右下に「図を表示」
...
- 箱の容積の「長方形の紙」パターン（`L3-06-rectangular-paper`）で、
...
- 利益と割引（L3-09）で、上下型分数（`x/10`）を使った式が入力・判定できること
...
- 段位認定モードで「2次方程式」とNORMAL・HARDを選択でき、対応する難易度のカテゴリ
  （NORMAL：連続する整数の積〜箱の容積、HARD：動点〜利益と割引）
  だけが出題されること
