...
- 到達済みのステージのみを表示する簡易マップ（未到達のステージは「？」）
- ボス撃破エンディング・敗走エンディング・リタイア結果画面（いずれも、出会った敵・獲得アイテム・
  たどったルート・正解数などの冒険の記録を表示）
...
アイテム・たどったルート・正解数・不正解数・ヒント使用回数・クリアした部屋数・単元）を
共有の画面（`quest-view-summary`）で表示する。
...
- ゴールデンパス（部屋A、必要正解数1・制限時間なし）：スタート→オープニング画面
  ...
  Aの`successRooms`どおり[C, D]の2択が表示されること→冒険マップに訪問済みのAだけが表示され、
  未到達のステージが「？」のままであること
...
- リタイア：敵出現画面の「リタイア」ボタンを押すと確認カード（「冒険をリタイアしますか？」/
  ...
  「リタイアする」を押すと、たどったルート（例：B → D → F）・獲得アイテムを含む結果画面が
  表示されること
- ボス戦・ステージ4→5の直行（A→C→F→L→Uの経路、いずれも必要正解数1・失敗条件なし）：
  ...
  エンディング画面に、たどったルート（A → C → F → L → U）・出会った敵5体・獲得アイテム5個が
  正しく表示されること
